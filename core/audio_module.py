...
class AudioModule:
    ...
    def process(self, audio: np.ndarray, sample_rate: int, reset: bool = False) -> np.ndarray:
        """Process audio with this module. Handles plugin instantiation and caching for real-time safety."""
        if not pedalboard: return audio
        
        # In standalone mode (outside HybridChain), we handle plugin caching here
        if self._cached_plugin is None or self._cached_sr != sample_rate:
            p = self.get_plugin(sample_rate)
            if p is not None:
                self._cached_plugin = p
                self._cached_sr = sample_rate
            else:
                return audio # This module has no plugin and didn't override process()
        else:
            self.update_plugin(self._cached_plugin, sample_rate)
            
        return self._cached_plugin.process(audio, sample_rate, reset=reset)

    def get_plugin(self, sample_rate: int) -> Any:
        """Return a pedalboard plugin instance for this module. Should be overridden."""
        return None

    def update_plugin(self, plugin: Any, sample_rate: int) -> None:
        """Update an existing plugin instance with current parameter values. Should be overridden."""
        pass
...
class CompressorModule(AudioModule):
    ...
    def update_plugin(self, plugin: Any, sample_rate: int) -> None:
        if not pedalboard or not isinstance(plugin, pedalboard.Pedalboard): return
        p = self.parameters
        
        # Accessing by index as created in get_plugin
        comp = plugin[0]
        gain_plugin = plugin[1]
        
        comp.threshold_db = p["Threshold"].current_value
        comp.ratio = p["Ratio"].current_value
        comp.attack_ms = max(2.0, p["Attack"].current_value)
        comp.release_ms = p["Release"].current_value
        ...
        gain_plugin.gain_db = p["Gain"].current_value + auto_makeup
...
class ExpanderModule(AudioModule):
    ...
    def update_plugin(self, plugin: Any, sample_rate: int) -> None:
        if not pedalboard or not isinstance(plugin, pedalboard.NoiseGate): return
        p = self.parameters
        plugin.threshold_db = p["Threshold"].current_value
        plugin.ratio = p["Ratio"].current_value
        plugin.attack_ms = p["Attack"].current_value
        plugin.release_ms = p["Release"].current_value
...
class TransientShaperModule(AudioModule):
    ...
    def __init__(self):
        ...
        # Pre-instantiate plugins for real-time safety
        if pedalboard:
            self._att_boost_comp = pedalboard.Compressor(threshold_db=-24.0, ratio=4.0, attack_ms=15.0, release_ms=50.0)
            self._att_cut_comp = pedalboard.Compressor(threshold_db=-24.0, ratio=4.0, attack_ms=1.0, release_ms=50.0)
            self._sus_boost_comp = pedalboard.Compressor(threshold_db=-24.0, ratio=4.0, attack_ms=1.0, release_ms=250.0)
            self._sus_cut_gate = pedalboard.NoiseGate(threshold_db=-36.0, ratio=2.0, attack_ms=10.0, release_ms=100.0)
        else:
            self._att_boost_comp = None
            self._att_cut_comp = None
            self._sus_boost_comp = None
            self._sus_cut_gate = None
    ...
    def process(self, audio: np.ndarray, sample_rate: int, reset: bool = False) -> np.ndarray:
        ...
        if att_db != 0:
            if att_db > 0:
                ...
                wet_audio = self._att_boost_comp.process(current_audio, sample_rate, reset=reset)
                ...
                wet_audio = self._att_cut_comp.process(current_audio, sample_rate, reset=reset)
        ...
        if sus_db != 0:
            if sus_db > 0:
                ...
                wet_audio = self._sus_boost_comp.process(current_audio, sample_rate, reset=reset)
                mix = min(1.0, sus_db / 20.0)
                makeup = 10 ** ((sus_db * 0.5) / 20.0)
                current_audio = (current_audio * (1.0 - mix)) + (wet_audio * mix * makeup)
            else:
                # Cut sustain: expander on the tail
                wet_audio = self._sus_cut_gate.process(current_audio, sample_rate, reset=reset)
...
class ClipperModule(AudioModule):
    ...
    def update_plugin(self, plugin: Any, sample_rate: int) -> None:
        if not pedalboard: return
        # Check if it's the right plugin type (pedalboard.Clipping might not exist in all versions)
        if hasattr(pedalboard, "Clipping") and isinstance(plugin, pedalboard.Clipping):
            plugin.threshold_db = self.parameters["Threshold"].current_value

    def process(self, audio: np.ndarray, sample_rate: int, reset: bool = False) -> np.ndarray:
        # We override process to provide a fast numpy-based hard-clipping fallback
        if not pedalboard:
            threshold_db = self.parameters["Threshold"].current_value
            threshold_linear = 10 ** (threshold_db / 20.0)
            return np.clip(audio, -threshold_linear, threshold_linear)
        
        # Use base class caching/plugin logic
        return super().process(audio, sample_rate, reset=reset)
...
class LimiterModule(AudioModule):
    ...
    def update_plugin(self, plugin: Any, sample_rate: int) -> None:
        if not pedalboard or not isinstance(plugin, pedalboard.Pedalboard): return
        p = self.parameters
        gain_plugin = plugin[0]
        limiter = plugin[1]
        gain_plugin.gain_db = p["Gain"].current_value
        limiter.threshold_db = p["Ceiling"].current_value
        limiter.release_ms = p["Release"].current_value
...
class ConvolutionModule(AudioModule):
    ...
    def update_plugin(self, plugin: Any, sample_rate: int) -> None:
        if not pedalboard or not isinstance(plugin, pedalboard.Convolution): return
        plugin.mix = self.parameters["Mix"].current_value
